...
int testLogger() {
  ...
  Logger::log(LogLevel::WARNING, LogType::Audio, "Audio file 'background.mp3' not found, using fallback.");
  ...
  Logger::log(LogLevel::DEBUG, LogType::AssetLoading, "Started loading asset bundle 'environment.pack'.");
  Logger::info("Custom control scheme loaded for gamepad.");
  Logger::log(LogLevel::WARNING, LogType::AssetLoading, "High-resolution texture 'forest.png' may affect performance.");
  Logger::error("Failed to compile vertex shader 'basic.vert'.");
  Logger::log(LogLevel::DEBUG, LogType::Scripting, "Player script attached to entity 'PlayerCharacter'.");
  Logger::log(LogLevel::DEBUG, LogType::Scripting, "Enemy script attached to entity 'EnemyController'.");
